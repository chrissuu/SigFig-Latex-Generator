...
string calculate(string args, int priority, bool useSigFigs)
{
    ...
    if (priority == 4)
    {
        ...
        while (args.find_last_of("(") != string::npos)
        {
            ...
            args = args.substr(0, args.find_last_of("(")) + calculate(temp, 3, useSigFigs) + args.substr(args.find(")") + 1, args.size());
            // cout << args << "\n";
            //  cout << "args: " << args << "\n";
        ...
        string temp = calculate(args, 3, useSigFigs);
    ...
    if (priority == 3)
    {
        ...
        return calculate(args, 2, useSigFigs);
    }
    ...
    if (priority == 2)
    {
        while (args.find("*") != string::npos)
        {
            ...
            {
                double prod = t.first * t.second;
                int firstIndex = temp.first.first;
                int lastIndex = temp.first.second;
                args = args.substr(0, firstIndex) + to_string(prod) + args.substr(lastIndex, args.size());
            }
        }
        ...
        while (args.find("/") != string::npos)
        {
            ...
            if (useSigFigs)
            {
                ...
                args = args.substr(0, firstIndex) + quot.getSVal() + args.substr(lastIndex, args.size());
            }
            else
            {
                double quot = t.first / t.second;
                int firstIndex = temp.first.first;
                int lastIndex = temp.first.second;
                args = args.substr(0, firstIndex) + to_string(quot) + args.substr(lastIndex, args.size());
            }
        }

        return calculate(args, 1, useSigFigs);
    }
    ...
    if (priority == 1)
    {
        while (args.find("+") != string::npos)
        {
            ...
            {
                double sum = t.first + t.second;

                int firstIndex = temp.first.first;
                int lastIndex = temp.first.second;

                args = args.substr(0, firstIndex) + to_string(sum) + args.substr(lastIndex, args.size());
            }
        }
        ...
        while (args.find("-") != string::npos && args[0] != '-')
        {
            ...
            {
                double diff = t.first - t.second;
                int firstIndex = temp.first.first;
                int lastIndex = temp.first.second;

                args = args.substr(0, firstIndex) + to_string(diff) + args.substr(lastIndex, args.size());
            }
        }
...
int main()
{
    ...
    s1 = calculate(s1,4, true);
    s2 = calculate(s2,4, true);
    s3 = calculate(s3,4, true);
    s4 = calculate(s4,4, true);
    s5 = calculate(s5,4, true);
    s6 = calculate(s6, 4, true);
